...
public class FormAtendimentoItem extends javax.swing.JInternalFrame implements IFormularioCadastro {
    ...
    private List<AtendimentoItem> lista;
    private EnumAcaoTela acaoTela;
    private EnumTipoPesquisa tipoPesquisa;
    private ModeloComboProduto modeloComboProduto;

    public FormAtendimentoItem(EnumAcaoTela acaoTela, IFormularioLista fl, AtendimentoItem atendimentoItem) {
        entrarTela();
        this.fl = fl;
        this.acaoTela = acaoTela;
        switch (acaoTela.getCodigo()) {
            case 1:
                this.atendimentoItem.setAtendimento(atendimentoItem.getAtendimento());
                break;
            case 2:
                this.atendimentoItem = atendimentoItem;
                this.atendimentoItem.setAtendimento(atendimentoItem.getAtendimento());
                this.txtPercentualDesconto.setText(atendimentoItem.getPercentualDesconto().toString());
                this.cboProduto.getModel().setSelectedItem(atendimentoItem.getProduto());
                this.txtQuantidade.setText(atendimentoItem.getQuantidade().toString());
                this.txtValorDesconto.setText(atendimentoItem.getValorDesconto().toString());
                this.txtValorTotal.setText(atendimentoItem.getValorTotal().toString());
                this.txtValorUnitario.setText(atendimentoItem.getValorUnitario().toString());
                break;
            case 3:
                this.atendimentoItem = atendimentoItem;
                this.atendimentoItem.setAtendimento(atendimentoItem.getAtendimento());
    ...
    private void btnSalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSalvarActionPerformed
        if (validarEntidade()) {
            switch (this.acaoTela.getCodigo()) {
                case 1:
                    atendimentoItemController.salvar(atendimentoItem);
                    JOptionPane.showMessageDialog(null, "AtendimentoItem inserido com sucesso", "SUCESSO",
                            JOptionPane.INFORMATION_MESSAGE);
                    break;
                case 2:
                    atendimentoItemController.alterar(atendimentoItem);
                    JOptionPane.showMessageDialog(null, "AtendimentoItem alterado com sucesso", "SUCESSO",
                            JOptionPane.INFORMATION_MESSAGE);
                    break;
            }
    ...
    public boolean validarEntidade() {
        atendimentoItem.setAtendimento(atendimentoItem.getAtendimento());
        atendimentoItem.setPercentualDesconto((double) txtPercentualDesconto.getValue());
        atendimentoItem.setProduto((Produto) cboProduto.getSelectedItem());
        atendimentoItem.setQuantidade((double) txtQuantidade.getValue());
        atendimentoItem.setValorDesconto((double) txtValorDesconto.getValue());
        atendimentoItem.setValorTotal((double) txtValorTotal.getValue());
        atendimentoItem.setValorUnitario((double) txtValorUnitario.getValue());
        return true;
    }
    ...
    public void entrarTela() {
        try {
            ...
            Logger.getLogger(FormAtendimentoItem.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
